 ### Start ALASSO
      ...
      beta_ini <- c(fit.all$coefficients[3:(p-1)])
	weight0=abs(1/beta_ini)
      ...
      # Newton-Rapshon 
      	...
      	betahat<-c(fit.all$coefficients)
#		betahat <- beta_lasso
      	...
      	repeat{
        	...
        	# 
        	#WL=diag(c((tun/abs(betahat+10^(-8)))*weight0))
        	WL=diag(c(0,0,c(tun/abs(betahat[3:(p-1)]+10^(-6)))*weight0,0))
